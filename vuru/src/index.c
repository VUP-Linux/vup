...
static int run_curl(const char *header_path, const char *output_path,
  ...
  if (pid == 0) {
    ...
    if (etag && etag[0] != '\0') {
      ...
      execlp("curl", "curl", "-s", "-L", "-D", header_path, "-o", output_path,
             "-H", etag_header, url, (char *)NULL);
    } else {
      execlp("curl", "curl", "-s", "-L", "-D", header_path, "-o", output_path,
             url, (char *)NULL);
    }
 ...
 */
static void parse_headers(const char *header_path, int *status, char *etag,
                          size_t etag_size) {
  if (!header_path || !status || !etag)
    return;
...
Index *index_load_or_fetch(const char *url, int force_update) {
  ...
  int ret =
      snprintf(index_path, sizeof(index_path), "%s/index.json", cache_dir);
  if (ret < 0 || (size_t)ret >= sizeof(index_path))
    return NULL;

  ret = snprintf(etag_path, sizeof(etag_path), "%s/index.json.etag", cache_dir);
  if (ret < 0 || (size_t)ret >= sizeof(etag_path))
    return NULL;

  ret = snprintf(header_path, sizeof(header_path), "%s/headers.txt", cache_dir);
  if (ret < 0 || (size_t)ret >= sizeof(header_path))
    return NULL;

  ret = snprintf(temp_index_path, sizeof(temp_index_path), "%s/index.json.tmp",
                 cache_dir);
  if (ret < 0 || (size_t)ret >= sizeof(temp_index_path))
    return NULL;
