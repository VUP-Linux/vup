 ...
 */
static int run_xbps_upgrade(const char *repo_url, const char *pkg_name,
                            int yes) {
  pid_t pid = fork();
  ...
  if (pid == 0) {
    if (yes) {
      execlp("sudo", "sudo", "xbps-install", "-R", repo_url, "-Su", "-y",
             pkg_name, (char *)NULL);
    } else {
      execlp("sudo", "sudo", "xbps-install", "-R", repo_url, "-Su", pkg_name,
             (char *)NULL);
    }
    _exit(127);
  }
...
static int parse_installed_pkg(const char *line, char *name, size_t name_size,
  ...
  if (full_len == 0 || full_len >= name_size)
    return 0;
  ...
  *dash = '\0';
  ...
  if (nlen >= name_size || vlen >= ver_size)
    return 0;
...
static int show_batch_review(UpgradeInfo *upgrades, int count) {
  ...
  int fd =
      diff_create_temp_file("vuru_review", review_path, sizeof(review_path));
  if (fd < 0) {
  ...
  for (int i = 0; i < count; i++) {
    fprintf(review, "  [%d] %s: %s -> %s\n", i + 1, upgrades[i].name,
            upgrades[i].installed_ver, upgrades[i].new_ver);
  }
  fprintf(review, "\n");
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (upgrades[i].cached_template) {
      char *diff =
          diff_generate(upgrades[i].cached_template, upgrades[i].new_template);
      if (diff) {
        fprintf(review, "%s\n", diff);
        free(diff);
      }
  ...
  if (fgets(input, sizeof(input), stdin)) {
    input[strcspn(input, "\n")] = '\0';
    if (input[0] == '\0' || strcasecmp(input, "y") == 0 ||
...
int xbps_upgrade_all(Index *idx, int yes) {
  ...
  while (fgets(line, sizeof(line), fp) && upgrade_count < MAX_UPGRADES) {
    ...
    if (!parse_installed_pkg(line, name, sizeof(name), installed_ver,
    ...
    if (!idx_ver || !cJSON_IsString(idx_ver) || !idx_ver->valuestring ||
        !repo_urls || !cJSON_IsObject(repo_urls) || !category ||
  ...
  for (int i = 0; i < upgrade_count; i++) {
    printf("  %s: %s -> %s\n", upgrades[i].name, upgrades[i].installed_ver,
           upgrades[i].new_ver);
  }
  printf("\n");
  ...
  if (!yes) {
    ...
    for (int i = 0; i < upgrade_count; i++) {
      upgrades[i].new_template =
          fetch_template(upgrades[i].category, upgrades[i].name);
