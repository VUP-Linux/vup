 ...
 */
static int run_xbps_install(const char *repo_url, const char *pkg_name,
                            int yes) {
  pid_t pid = fork();
  ...
  if (pid == 0) {
    if (yes) {
      execlp("sudo", "sudo", "xbps-install", "-R", repo_url, "-S", "-y",
             pkg_name, (char *)NULL);
    } else {
      execlp("sudo", "sudo", "xbps-install", "-R", repo_url, "-S", pkg_name,
             (char *)NULL);
    }
    _exit(127);
  }
...
int xbps_install_pkg(Index *idx, const char *pkg_name, int yes) {
  ...
  if (!cat || !cJSON_IsString(cat) || !cat->valuestring || !repo_urls ||
      !cJSON_IsObject(repo_urls)) {
  ...
  if (!url || !cJSON_IsString(url) || !url->valuestring) {
    log_error("Package '%s' is not available for architecture '%s'", pkg_name,
              arch);
    return -1;
  }

  log_info("Found %s in category '%s' for %s", pkg_name, cat->valuestring,
           arch);
