...
make_error :: proc(kind: Error_Kind, error_ctx: string = "", allocator := context.allocator) -> Error {
	return Error{
		kind = kind,
...
get_hint :: proc(kind: Error_Kind, ctx: string = "") -> string {
	...
	case .Package_Not_Found:
		return fmt.tprintf(
			...
			"  • Search all repos: xbps-query -Rs %s\n" +
	...
	case .Package_Not_In_VUP:
		return fmt.tprintf(
			"'%s' is not in VUP, but may be in official repos.\n" +
			"  • Try: sudo xbps-install %s\n" +
			"  • Or search: xbps-query -Rs %s",
			ctx, ctx, ctx,
		)
	...
	case .Dependency_Cycle:
		return "Circular dependencies detected. This is likely a packaging bug.\n" +
			"  • Report issue: https://github.com/VUP-Linux/vup/issues"
	
	case .Dependency_Conflict:
		return "Conflicting package versions required.\n" +
			"  • Try updating first: vuru -u\n" +
			"  • Check for held packages: xbps-pkgdb -m"
	
	case .Index_Fetch_Failed:
		return "Could not download package index.\n" +
			"  • Check internet connection\n" +
			"  • Try again: vuru -S"
	
	case .Index_Parse_Failed:
		return "Package index is corrupted.\n" +
			"  • Force refresh: vuru -S\n" +
			"  • Clear cache: rm -rf ~/.cache/vup"
	
	case .VUP_Repo_Not_Found:
		return "VUP repository not cloned.\n" +
			"  • Clone it: vuru clone\n" +
			"  • Default location: ~/.local/share/vup"
	
	case .Xbps_Src_Not_Found:
		return "xbps-src not found in current directory.\n" +
	...
	case .Build_Deps_Missing:
		return "Build dependencies are missing.\n" +
	...
	case .Permission_Denied:
		return "Operation requires root privileges.\n" +
