...
clone_run :: proc(args: []string, config: ^Config) -> int {
	...
	if !utils.mkdir_p(utils.path_join(home, ".local/share", allocator = context.temp_allocator)) {
		utils.log_error("Failed to create directory")
		return 1
	}

	if builder.vup_clone_or_update(target) {
		utils.log_info("VUP repository ready at %s", target)
		return 0
	}

	utils.log_error("Failed to clone/update VUP repository")
	return 1
}
